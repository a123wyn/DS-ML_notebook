...
class Softmax:
    ...
    def train(self, X, y):
        ...
        n_classes = len(np.unique(y)) # y种类数
        # 随机初始化权重矩阵
    ...
    def sgd(self, X, y, config):
        ...
        loss, dW = self.sample_and_calculate_gradient(X, y, self.W, 0, reg_strength)
    ...
    def sample_and_calculate_gradient(self, X, y, w, b, reg_strength):
        # random_indices = random.sample(range(X.shape[0]), batch_size)
        # X_batch = X[random_indices]
        # y_batch = y[random_indices]
        return self.loss(X, y, w, b, reg_strength)
